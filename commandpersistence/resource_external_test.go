...
resource "commandpersistence_cmd" "test" {
  program = ["%s", "cheese"]
  query = {
    value = "pizza"
  }
...
output "query_value" {
  value = commandpersistence_cmd.test.result["query_value"]
}
output "argument" {
  value = commandpersistence_cmd.test.result["argument"]
}
`

func TestResourceCmd_basic(t *testing.T) {
...
func TestResourceCmd_error(t *testing.T) {
	programPath, err := buildDataSourceTestProgram()
	if err != nil {
		t.Fatal(err)
		return
